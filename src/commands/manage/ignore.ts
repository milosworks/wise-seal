...
export default class Ignore extends Command {
	async run(ctx: CommandContext<typeof options>) {
		...
		if (channel) {
			...
			if (!doc) {
				doc = (await db.insert(guilds).values({ id: ctx.guildId! }).returning())![0]
			}
			if (doc.ignoredChannels.includes(channel.id)) return ctx.write({ content: 'This channel is already ignored' })
			doc.ignoredChannels.push(channel.id)

			await db.update(guilds).set({ ignoredChannels: doc.ignoredChannels }).where(sql`id = ${doc.id}`)
		...
		if (user) {
			...
			if (!doc) {
				doc = (await db.insert(users).values({ id: user.id }).returning())![0]
			}
			if (doc.ignoreCount) return ctx.write({ content: 'This user is already ignored' })

			doc.ignoreCount = true

			await db.update(users).set({ ignoreCount: doc.ignoreCount }).where(sql`id = ${doc.id}`)
